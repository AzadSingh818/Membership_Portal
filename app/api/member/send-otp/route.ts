  ...
  try {
    ...
    try {
      ...
      await sql`
        ...
        VALUES (
          ${memberData.phone}, ${otp}, 'phone', ${expiresAt.toISOString()}, NOW()
        )
      `
    } catch (dbError) {
    ...
    // Send SMS
    const smsSent = await sendPhoneSMS(memberData.phone, otp)
...
// app/api/member/verify-otp/route.ts - FIXED VERSION
import { NextRequest, NextResponse } from 'next/server'
import { sql } from '@vercel/postgres'
